...
async function queries() {
  ...
  try {
    ...
    const gt_one = await col.find({chg_pct: {$gt: 0.99}}).sort({chg_pct: -1}).limit(10).toArray()
    const lt_one = await col.find({chg_pct: {$lt: -0.99}}).sort({chg_pct: 1}).limit(10).toArray()
    const rel_vol = await col.find({relative_volume: {$gt: 2}}).sort({relative_volume: -1}).limit(10).toArray()
    const yld = await col.find({yield: {$gt: 6}, last: {$gt: 17}}).sort({yield: -1}).limit(10).toArray()
